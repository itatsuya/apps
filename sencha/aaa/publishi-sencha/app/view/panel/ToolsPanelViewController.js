...
Ext.define('RolandKioskPrint.view.panel.ToolsPanelViewController', {
 ...
 // Hidden to start with
    ...
    initialize: function(customSize) {
        ...
        for (var i = 0, length = toolButtons.length
 i < length; i++) {
            toolButtons[i].toggle(false)
 // Unpress all tool buttons.
        }
        ...
        showTools.btnBackgroundTool = showProductModelDesignerTools && !importedFromSVG
 // No background tool if importing from SVG
        ...
        if (!this.onCanvasSelectionChange) {
            this.onCanvasSelectionChange = {
                ...
                'canvas:x-ready': function() {
                    me.updateTools()
 // Update the tools state once canvas is ready
                },
            }
    ...
    canPlaceImagesOrClipartSelection: function(selectedObjects) {
        for (var i = 0, length = selectedObjects.length
 i < length; i++) {
