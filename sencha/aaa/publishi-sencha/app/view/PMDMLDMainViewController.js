...
Ext.define('RolandKioskPrint.view.PMDMLDMainViewController', {
 ...
 // viewBox is required
        ...
        if (svgElem.hasAttribute('id')) {
            ...
            var groupElemList = svgElem.getElementsByTagName('g')
 // get all root level groups
        ...
        var elems = svgDoc.getElementsByTagName('*')
 // Get all elements
 ...
 i < length; i++) {
        ...
        for (var i = 0, length = elemList.length
 i < length; i++) {
            var elem = elemList[i]
 ...
 i < numCommand; i++) {
                ...
                if (currentPath[0] != 'v' && currentPath[0] != 'V') {
                    for (var j = 1, length = currentPath.length
 j < length; j++){
    ...
    createPathFromPoints: function(points, scaleX, scaleY) {
        ...
        for (var i = 0, length = points.length
 i < length; i++) {
            var px = points[i].x,
                py = points[i].y
 ...
 // Nothing to refresh
        ...
        var me = this
 // Ext.Promise.then gets window scope by default
 ...
 // Not mapped
            ...
            for (var i = 0, length = connectedLayouts.length
 i < length; i++) {
                if (i != index) {
 ...
 i < length; i++) {
    ...
    initializePrintTypeFilter: function(component, filterFn) {
        ...
        if (features.allowFoilPrint || features.allowMetalPrint || features.allowCutPrint) {
            ...
            allowedFilters.push(colorPrint)
 // Color is always there
            ...
            printTypeFilter = new Ext.util.Filter({
                ...
                filterFn: function(record) {
                    ...
                    if (Array.isArray(recPrintType)) {
                        ...
                        for (var i = 0
 !hasPrintType && i < recPrintType.length; ++i) {
