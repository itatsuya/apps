...
Ext.Boot = Ext.Boot || (function (emptyFn) {
 ...
 Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/42.0.2311.135 Safari/537.36 Edge/12.10240"
 ...
 i < paramsArray.length; i++) {
                ...
                if (params.platformTags) {
                    ...
                    for (tmplen = tmpArray.length, i = 0
 i < tmplen; i++) {
...
//                     _debug("fetching " + url + " " + (async ? "async" : "sync"))
        ...
        getLoadIndexes: function (item, indexMap, loadOrder, includeUses, skipLoaded) {
            ...
            while (item = queue.shift()) {
                ...
                if (!(skipLoaded && entry.done)) {
                    ...
                    for (i = 0, len = dependencies.length
 i < len; i++) {
        ...
        getPathsFromIndexes: function (indexes, loadOrder) {
            ...
            for (index = 0, len = indexes.length
 index < len; index++) {
                if (indexes[index]) {
 ...
 i < len; i++) {
                ...
                for (t = 0, tlen = tmpExpanded.length
 t < tlen; t++) {
                    tUrl = tmpExpanded[t]
